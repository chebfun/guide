...
% Lloyd N. Trefethen, November 2009, last revision December 2013

%% 5.1  Complex functions of a real variable
% One of the attractive features of MATLAB(R) is that it handles complex
...
%%
% In MATLAB(R), both the variables |i| and |j| are initialized as the
% imaginary unit $i\equiv\sqrt{-1}$, but this code uses |1i| instead
% (just as one might write, for example, $3+2i$ or $2.2-1.1i$).
% Writing the imaginary unit in this fashion is a common trick among
% MATLAB(R) programmers, for it avoids the risk of surprises caused by
% |i| or |j| having been overwritten by other values. The
% |<http://www.mathworks.com/help/matlab/ref/axis.html axis equal>| command ensures that the real and
...
% An easier method is to use the *|<../../chebdoc/doc/chebjoin_DOC.html chebjoin>|* command,
% in which a construction like |<../../chebdoc/doc/chebjoin_DOC.html chebjoin>(f,g,h)|
% constructs a single chebfun with the same values as |f|, |g|, and |h|,
% but on a domain concatenated together.  Thus if the domains of |f|,
% |g|, |h| are $[a,b]$, $[c,d]$, and $[e,d]$, then
% |<../../chebdoc/doc/chebjoin_DOC.html chebjoin>(f,g,h)| has three pieces with domains
% $[a,b]$, $[b,b+(d-c)]$, $[b+(d-c),b+(d-c)+(d-e)]$.  Using this trick, we
% can construct the chebfun |z| above in the following alternative manner:
  s = chebfun(@(s) s,[0 1]);
...
%%
% Here we see what happens to |R| and |X| under the maps
% $z \mapsto z^2$ and $z \mapsto e^z$:
clf
...
% map $w = (1+z)^{-1}$, and the image of the image under the same map, and the
% image of the image of the image.  We also plot the limit point given by
% the equation $z = (1+z)^{-1}$, i.e., $z = \frac12 (\sqrt5-1)$.
moebius = @(z) 1./(1+z);
s = chebfun(@(s) s,[0 1]);
...
%%
% Here's a prettier version of the same image using the Chebfun
% *|<../../chebdoc/doc/fill_DOC.html fill>|* command.
S = join(-.5i+s, 1-.5i+1i*s, 1+.5i-s, .5i-1i*s);
clf
...
%%
% For example, in the example at the end of <guide5.html Section 5.1>
% the contour consists of two straight segments that begin at $0$ and
% end at $-1+0.5i$.  We can compute the integral of $e^{-z^2}$ over
...
% points $z_1$ and $z_2$ is path-independent.  To verify this, we can
% compute the same integral over the straight segment going directly
% from $0$ to $-1+0.5i$:
...
% *Cauchy integral formula*, $1/(2\pi i)$ times the integral of a
% meromorphic function $f$ around a closed contour is equal to the sum
% of the residues of $f$ associated with any poles it may have in the
% enclosed region. The *residue* of $f$ at a point $z_0$ is the
% coefficient of the degree $-1$ term in its Laurent expansion at $z_0$.
% For example, the function $e^z / z^3$ has Laurent series
% $z^{-3} + z^{-2} + \frac12 z^{-1} + \frac16 z^0 + \cdots$
% at the origin, and so its residue there is $1/2$. We can confirm
% this by computing the contour integral around a circle:
...
% periodicity, and the loss in efficiency is only about a factor of
% $\pi/2$ [Hale & Trefethen 2008].
...
%%
% The integral of $f(z) = \log(z) \tanh(z)$ around this contour will
% be equal to $2\pi i$ times the sum of the residues at the poles
% of $f$ at $\pm 0.5\pi i$.
f = log(z).*tanh(z);
I = sum(f.*diff(z))
...
% $z/(e^z-1)$. Here is $B_{10}$ compared with its exact value $5/66$.
k = 10;
...
%%
% Notice that we have taken $z$ to be a circle of radius $5$. If the
% radius is $1$, the accuracy is a good deal lower:
z = chebfun('exp(1i*s)',[0 2*pi]);
f = z./((exp(z)-1));
B10 = factorial(k)*sum((f./z.^(k+1)).*diff(z))/(2i*pi)
...
%
% \[  N = \frac{1}{2\pi i} \int \frac{f'}{f} dz,  \]
%
% where the integral is taken over the boundary.  Since
% $f' = df/dz = (df/ds)(ds/dz)$, we can rewrite this as
%
% \[  N = \frac{1}{2\pi i} \int \frac{(df/ds)}{f} ds.  \]
%
% For example, the function $f(z) = \sin^3(z) + \cos^3(z)$ clearly has no
% poles; how many zeros does it have in the disk about $0$ of radius $2$?
...
% number is with the Chebfun overloads of the MATLAB(R) commands
% *|<../../chebdoc/doc/angle_DOC.html angle>|* and *|<../../chebdoc/doc/unwrap_DOC.html unwrap>|*:
...
%
% \[  r = \frac{1}{2\pi i} \int \frac{z(df/ds)}{f} ds  \]
%
...
%% 5.5 Alphabet soup
% The Chebfun command *|<../../chebdoc/doc/scribble_DOC.html scribble>|* returns a
% piecewise linear complex chebfun corresponding to a word spelled
% out in capital letters.  For example:
...
%%
% Though its applications are unlikely to be mathematical, $f$ is a
% precisely defined mathematical object just like any other chebfun.
% If we wish, we can compute with it:
f(0), norm(f)
...
% second column of a Chebfun quasimatrix -- see <guide6.html Chapter 6>.)
L = f.ends(end);
...
% <http://www2.maths.ox.ac.uk/chebfun/examples/fun/html/Birthday.shtml 
% "Birthday cards and analytic function">
% in the online
% <http://www2.maths.ox.ac.uk/chebfun/examples/ Chebfun Examples collection>,
% and further related explorations in the
% <http://www2.maths.ox.ac.uk/chebfun/examples/geom/ Geometry section>.

%% 5.6  References
%
% [Austin, Kravanja & Trefethen 2013] A. P. Austin, P. Kravanja and
% L. N. Trefethen, "Numerical algorithms based on analytic function
% values at roots of unity," SIAM J. Numer. Anal., submitted, 2013.
%
