...
% the command *|<#CHEBDOC:roots roots>|*:
  x = chebfun('x');
  p = x.^3 + x.^2 - x;
  r = roots(p)
...
% The MATLAB(R) |<#MATLABDOC:roots roots>|
...
% on <#MATHWORLD:AiryFunctions Airy functions at Wolfram MathWorld>.
...
% |<#CHEBDOC:min min>(f,g)| or |<#CHEBDOC:max max>(f,g)|.
...
%%
% The function |<#CHEBDOC:sign sign>| also introduces breaks,
% as illustrated in the last section. The commands
% |<#CHEBDOC:round round>|, |<#CHEBDOC:floor floor>|,
% and |<#CHEBDOC:ceil ceil>| behave like this too. For example,
...
% If |<#CHEBDOC:min min>| or |<#CHEBDOC:max max>| is applied to a
...
% find all its zeros. The alternative function |<#CHEBDOC:minandmax minandmax>|
% computes both at once:
  [extemevalues,extremepositions] = minandmax(f)

%% 3.5 |norm(f,1)| and |norm(f,inf)|
% The default, 2-norm form of the *|<#CHEBDOC:norm norm>|* command was
% considered in <guide2.html Section 2.2>. In standard MATLAB(R) one
% can also compute 1-, infinity-, and Frobenius norms with
% |<#MATLABDOC:norm norm>(f,1)|, |<#MATLABDOC:norm norm>(f,inf)|,
% and |<#MATLABDOC:norm norm>(f,'fro')|.  These have been
...
% The 1-norm |<#CHEBDOC:norm norm>(f,1)| of a chebfun |f| is the
% integral of the absolute value of |f|, and Chebfun computes this
% by adding up segments between zeros, at which |<#CHEBDOC:abs abs>(f)|
...
% |<#CHEBDOC:roots roots>| command makes this possible in various ways
...
% has <#CHEBDOC:length length> $L$, the Chebfun ellipse associated with a
...
% |<#CHEBDOC:roots roots>(g,'complex')| first effectively does
% |<#CHEBDOC:roots roots>(g,'all')|, then returns only those roots lying
