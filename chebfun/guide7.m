...
% described in Section 7.9 and in Chapter 10, and for PDEs, try
% `help pde15s`.
...
%%
% Like chebfuns, chebops start from premise of approximation by
% piecewise polynomial interpolants; in the context of
...
% [Birkisson & Driscoll 2011, Driscoll & Hale 2014] . Much of the
% functionality of linear chebops is actually implemented in various classes
% built around the idea of what we call a "linop",
% but users generally do not deal with linops and related 
% structures directly.
...
%%
% For example, suppose we want to solve the differential equation $u''+x^3u
...
% At the moment Chebfun does not include truly periodic spectral
% discretizations based on Fourier series, though we
...
% form $Au = \lambda Bu$.  For these one must specify two linear chebops `A`
...
%% 
% Let us say a word about how Chebfun carries out these
% computations.  Until Chebfun version 5,
...
% boundary conditions.
% When a differential equation is solved in Chebfun,
% the problem is solved on a sequence of grids of size $33, 65, \dots$ until
...
% automatically according to the intrinsic resolution requirements.
...
% differential equations in Chebfun's default mode is rarely close to machine precision.
...
% *ultraspherical* or *Olver-Townsend* spectral discretizations
% [Olver & Townsend 2013].  Here, rather
% than a single Chebyshev basis, several different bases of ultraspherical
% polynomials are used, depending on the order of the differential operator.
% This leads to better conditioned matrices that are also sparser, especially
% for linear problems with constant or smooth coefficients.  By default,
% Chebfun uses rectangular collocation discretizations,
% but most problems can equally
% be solved in ultraspherical mode, and the results may be more accurate. 
% For example, here we solve a problem whose exact solution is $\cos(x)$
% in the rectangular fashion and check the error at $x=5$:
tic
...
%%
% We can switch to ultraspherical mode and run the same experiment
% again like this:
...
% (because of the derivative) whereas the dependences of $w$ on $v$ and $y$ on
% $u$ are local (diagonal).  
