...
% here is a polynomial with two roots in $[-1,1]$, which we compute with
% the command *|<../../chebdoc/doc/roots_DOC.html roots>|*:
  x = chebfun('x');
  p = x.^3 + x.^2 - x;
  r = roots(p)
...
% The MATLAB(R) |<http://www.mathworks.com/help/matlab/ref/roots.html roots>|
% command works from a polynomial's coefficients and computes
...
% considered in Sections <guide1.html 1.2> and <guide2.html 2.1>.
% Here is a similar calculation for
% the Airy functions $\mathrm{Ai}$ and $\mathrm{Bi}$, modeled after the page
% on <http://mathworld.wolfram.com/AiryFunctions.html Airy functions at Wolfram MathWorld>.
...
%%
% Here for example are the three roots of $\mathrm{Ai}$ and $\mathrm{Bi}$
% closest to $0$:
...
% then found as eigenvalues of a _colleague matrix_, the analogue for
...
% problems.  For example, where do the curves $x$ and $\cos(x)$ intersect?
% Here is the answer.
  x = chebfun('x',[-2 2]);
  hold off, plot(x)
  f = cos(x);
...
% put the discontinuities. For example, the |<../../chebdoc/doc/abs_DOC.html abs>| command
% introduces breakpoints wherever the argument goes through zero.
% Here we see that |x| consists of a single piece, whereas
% |<../../chebdoc/doc/abs_DOC.html abs>(x)| consists of two pieces.
  x = chebfun('x')
  absx = abs(x)
...
%%
% We saw this effect already in <guide1.html Section 1.4>.
% Another similar effect shown in that section occurs with
% |<../../chebdoc/doc/min_DOC.html min>(f,g)| or |<../../chebdoc/doc/max_DOC.html max>(f,g)|.
% Here, breakpoints are introduced at points where |f-g| is zero:
  f = min(x,-x/2), subplot(1,2,1), plot(f,'.-')
  g = max(.6,1-x.^2), subplot(1,2,2), plot(g,'.-'), ylim([.5,1])

%%
% The function |<../../chebdoc/doc/sign_DOC.html sign>| also introduces breaks,
% as illustrated in the last section. The commands
% |<../../chebdoc/doc/round_DOC.html round>|, |<../../chebdoc/doc/floor_DOC.html floor>|,
% and |<../../chebdoc/doc/ceil_DOC.html ceil>| behave like this too. For example,
% here is $e^x$ rounded to nearest multiples of $0.5$:
  g = exp(x);
  clf, plot(g)
%%
% FIXME: |round| not implemented
%%
%    gh = 0.5*round(2*g);
%    hold on, plot(gh,'r');
%    grid on
...
% positive:
  ...
  plot(minima,h(minima),'ok','markersize',12)

%% 3.4 Global extrema: |max| and |min|
% If |<../../chebdoc/doc/min_DOC.html min>| or |<../../chebdoc/doc/max_DOC.html max>| is applied to a
% single chebfun, it returns its global minimum or maximum. For example:
  f = chebfun('1-x.^2/2');
  [min(f) max(f)]
...
%%
% As with standard MATLAB(R), one can find the location of the extreme point
...
% find all its zeros. The alternative function |<../../chebdoc/doc/minandmax_DOC.html minandmax>|
% computes both at once:
  [extemevalues,extremepositions] = minandmax(f)

%% 3.5 |norm(f,1)| and |norm(f,inf)|
% The default, 2-norm form of the *|<../../chebdoc/doc/norm_DOC.html norm>|* command was
% considered in <guide2.html Section 2.2>. In standard MATLAB(R) one
% can also compute 1-, infinity-, and Frobenius norms with
% |<http://www.mathworks.com/help/matlab/ref/norm.html norm>(f,1)|, |<http://www.mathworks.com/help/matlab/ref/norm.html norm>(f,inf)|,
% and |<http://www.mathworks.com/help/matlab/ref/norm.html norm>(f,'fro')|.  These have been
...
% chebfun but differ for quasimatrices; see <guide6.html Chapter 6>.)
% The 1-norm |<../../chebdoc/doc/norm_DOC.html norm>(f,1)| of a chebfun |f| is the
% integral of the absolute value of |f|, and Chebfun computes this
% by adding up segments between zeros, at which |<../../chebdoc/doc/abs_DOC.html abs>(f)|
% will typically have a discontinuous slope. The infinity-norm is
% computed from the formula |norm(f, inf) = max(max(f), -min(f))|.
...
% Sometimes we may want to get our hands on these roots, and the
% |<../../chebdoc/doc/roots_DOC.html roots>| command makes this possible in various ways
% through the flags *|'all'|*, *|'complex'|*, and *|'norecurse'|*.
...
% has <../../chebdoc/doc/length_DOC.html length> $L$, the Chebfun ellipse associated with a
% parameter $\delta\ll1$ is the region in the complex plane bounded
% by the image under the map $\frac12 (z+z^{-1})$ of the
% circle $|z|=r$, where $r$ is defined by $r^{-L}=\delta$.
% (See Chapter 8 of [Trefethen 2013].)  The command
% |<../../chebdoc/doc/roots_DOC.html roots>(g,'complex')| first effectively does
% |<../../chebdoc/doc/roots_DOC.html roots>(g,'all')|, then returns only those roots lying
...
% it may be safer to add a flag *|'norecurse'|* to the roots call, at the cost
...
%%
% To find poles in the complex plane as opposed to zeros, see
% <guide4.html Section 4.8>.
