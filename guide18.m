...
% specified by a $r_1\times r_2\times r_3$ _core tensor_ called `core`.  
% In short, we write:
...
g = chebfun3(@(x,y,z) cos(15*exp(z))./(5+x+2*y+z));
...
% This is a quasimatrix with 11 columns:
size(g.cols)
...
% representing smooth functions which are periodic in all three variables. 
...
% interpolants through 143, 3, and 13 equally spaced samples over $[-\pi,\pi)^3$
% in the $x$, $y$, and $z$ directions, respectively. In other words,
% trigonometric polynomials of degrees 71, 1, and 6 are enough to resolve 
...
%%
% For example, the following function is equal to $x^2$ plus a harmonic function:
f = chebfun3(@(x,y,z) x.^2 + 1./sqrt(x.^2 + y.^2 + (2-z).^2))

%%
% [Remark to Chebfunners: a harmonic function gives scaling trouble... to be discussed.]
...
F = grad(f);
curve = chebfun(@(t) [t.*cos(t) t.*sin(t) t], [0, 5*pi]); % Conical spiral
plot3(curve(:,1), curve(:,2), curve(:,3),'b'), shg
...
%
% [Hashemi & Trefethen 2016] B. Hashemi and L. N. Trefethen, manuscript in 
% preparation.
%
