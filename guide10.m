...
% extensions of chebops to the nonlinear case, as well as other methods for
% nonlinear ODEs:
%
...
% ODE23, ODE45, ODE113, ODE15S, ODE23S, ODE23T, and ODE23TB, and are adapted to
% various mixes of accuracy requirements and stiffness.

%%
% Chebfun includes overloads of ODE45 (for medium accuracy), ODE113 (for high
...
% the chebfun overload of ODE45 rather than the Matlab original.
...
%%
% As a second example let us consider the linear second-order equation u"=-u,
% whose solutions are sines and cosines.  We convert this to first-order form by
% using a vector v with v(1)=u and v(2)=u', and solve the problem again using
% ode45:
fun = @(t, v) [v(2); -v(1)];
...
% these too have been overloaded in Chebfun. Again the Chebfun usage becomes
...
% documentation.  The domain is [0,4], the equation is u'' + abs(u) = 0, and the
% boundary conditions are u(0)=0, u(4)=-2.  We get one solution from the initial
% condition u=1:
...
%
% In Chebfun, to compute this operator, we need to decide a variable to act as a
% basis variable for derivative computations, and seed its derivative. To
% compute derivatives with respect to u, we convert it to an ADCHEBFUN, and redo
% the computations:
u = adchebfun(u);
v = exp(x) + u.^3;
w = u + diff(v);
%%
% We can now obtain the derivative of v with respect to u by accessing the
% .jacobian field of v:
dvdu = v.jacobian;

%%
% The result dvdu is a linear chebop of the kind discussed in Chapter 7. For
% example, dvdu*x is 3x^4 times x, or 3x^5:
plot(dvdu*x, LW, lw)
...
%%
% Notice that one of the fields is called init, which may hold an initial guess
...
% BVP 0.001u''-u^3 = 0, u(-1) = 1, u(1) = -1. However, since the required
% Jacobian information is now computed by AD, construction of the Jacobian
% operator J is taken care of by linearize(L,u), which returns the derivative of
% the operator J when it is linearized around the function u. Compare the code
